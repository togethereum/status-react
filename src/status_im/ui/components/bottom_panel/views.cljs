...
(views/defview animated-bottom-panel [val view on-close show-overlay?]
  (views/letsubs [{window-height :height} [:dimensions/window]]
    [bottom-panel (when val (select-keys val [:from :contact :amount :token :approve? :message :cancel? :hash :name :url :icons :wc-version :params :connector :description :topic :relay :self :peer :permissions :state])) view window-height on-close (if-not (nil? show-overlay?) show-overlay? true)]))
